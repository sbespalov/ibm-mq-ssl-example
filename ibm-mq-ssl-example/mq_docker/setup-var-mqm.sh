...
if [ -d "/var/mqm/qmgrs" ]; then
  ...
  echo "LOG setup-var.mqm.sh: Using existing MQ Data under /var/mqm"
  /opt/mqm/bin/crtmqdir -a -f
else
  if [ -L "/var/mqm" ]; then
    echo "LOG setup-var.mqm.sh: /var/mqm is already a symlink."
    /opt/mqm/bin/crtmqdir -a -f
  else
    if [ -d "/mnt/mqm/" ]; then
      DATA_DIR=/mnt/mqm/data
      MOUNT_DIR=/mnt/mqm
      echo "LOG setup-var.mqm.sh: Symlinking /var/mqm to $DATA_DIR"
